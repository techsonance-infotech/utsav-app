...
export async function PATCH(
  ...
  // 1. Role gate: only owner, admin, and treasurer can reject expenses
  ...
  try {
    ...
    if (updateError || !updatedExpense) {
      return NextResponse.json({ message: updateError?.message || "Failed to reject expense" }, { status: 500 });
    }
