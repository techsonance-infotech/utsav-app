...
export async function generateMetadata({ params }: PublicNewsPageProps) {
  ...
  return {
    title: tenant?.name ? `${tenant.name} | News & Updates 🪔` : "Mandal News",
    description: `Stay updated with the latest bulletins, spiritual updates and events of ${tenant?.name || "Mandal"}`,
  };
}
...
export default async function PublicNewsPage({ params }: PublicNewsPageProps) {
  ...
  // Fetch all published news from the public API
  let news: any[] = [];
  try {
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_APP_URL || "http://localhost:3000"}/api/v1/public/news?slug=${params.slug}`,
      { cache: "no-store" }
    );
    if (res.ok) {
      news = await res.json();
    ...
    console.error("Failed to fetch public news from API:", err);
  }
        ...
        <section className="relative min-h-[380px] flex items-center overflow-hidden py-16 bg-gradient-to-br from-[#2d1b0d] via-[#1e140a] to-[#140d06] text-white">
          ...
          <div className="relative z-10 px-6 max-w-7xl mx-auto w-full text-center space-y-5">
            <div className="inline-flex items-center gap-2 px-3.5 py-1.5 bg-white/5 backdrop-blur-md rounded-full border border-white/10">
              <span className="material-symbols-outlined text-sm text-[#ff9500] animate-pulse">campaign</span>
              <span className="text-[10px] uppercase tracking-widest text-[#ffdcbf] font-black">Bulletins</span>
            </div>
            <h1 className="text-3xl md:text-5xl font-extrabold tracking-tight uppercase leading-tight font-headline-lg max-w-3xl mx-auto">
              News & <span style={{ color: primaryColor }}>Bulletins</span>
            </h1>
            <p className="text-xs md:text-sm text-gray-300 max-w-lg mx-auto leading-relaxed">
              Stay up-to-date with Mandal announcements, administrative releases, emergency notes, and holiday updates.
            </p>
          </div>
        </section>

        {/* 2. News Bulletins Content */}
        <section className="py-16 px-6 max-w-7xl mx-auto w-full">
          {news && news.length > 0 ? (
            <NewsGrid news={news} primaryColor={primaryColor} />
          ) : (
            <div className="bg-white border border-dashed border-[#E8E2D6] p-16 rounded-[32px] text-center max-w-md mx-auto space-y-4">
              <span className="text-5xl block">📰</span>
              <h4 className="font-black text-sm text-[#3A3530] uppercase">No Bulletins Found</h4>
              <p className="text-xxs text-gray-500 font-semibold leading-relaxed">
                There are no published news updates or announcements for this Mandal currently.
              </p>
            </div>
          )}
        </section>
